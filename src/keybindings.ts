...
export class Keybindings {
    ...
    constructor(ext: Ext) {
        ...
        this.window_focus = {
            "focus-left": () => {
                this.stack_select(
                    ext,
                    (id, stack) => id === 0 ? null : stack.windows[id - 1].entity,
            ...
            "focus-right": () => {
                this.stack_select(
                    ext,
                    (id, stack) => stack.windows.length > id + 1 ? stack.windows[id + 1].entity : null,
            ...
            "toggle-stacking": () => ext.auto_tiler?.toggle_stacking(ext),
    ...
    stack_select(
        ...
        const switched = this.stack_switch(ext, (stack) => {
            ...
            const id = stack.container.active_id;
            if (id === -1) {
                // TODO: Move window out of stack
            } else {
                const next = select(id, stack.container);
                if (next) {
                    stack.container.activate(next);
